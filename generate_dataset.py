...
def load_seeds(seed_dir: Path):
    ...
    with open(seed_dir / "merchants_seed.json", "r", encoding="utf-8") as fp:
        merchants = json.load(fp)["merchants"]
    with open(seed_dir / "customers_seed.json", "r", encoding="utf-8") as fp:
        customers = json.load(fp)["customers"]
    with open(seed_dir / "triggers_seed.json", "r", encoding="utf-8") as fp:
    ...
    for m in merchants:
        ...
        for i in range(max(0, target_per_merchant - cur)):
            ...
            expanded.append({
                ...
                "consent": {"opted_in_at": "2025-09-01", "scope": ["promotional_offers", "recall_reminders"]},
            })
            next_idx += 1
    return expanded
